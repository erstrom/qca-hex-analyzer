...
class WmiTlvMsgInit(WmiTlvMsg):
    ...
    def __init__(self, data):
        ...
        scan_max_pending_reqs = _create_le32(cfg_data[56:])
        bmiss_offload_max_vdev = _create_le32(cfg_data[60:])
        roam_offload_max_vdev = _create_le32(cfg_data[64:])
        roam_offload_max_ap_profiles = _create_le32(cfg_data[68:])
        num_mcast_groups = _create_le32(cfg_data[72:])
        num_mcast_table_elems = _create_le32(cfg_data[76:])
        mcast2ucast_mode = _create_le32(cfg_data[80:])
        tx_dbg_log_size = _create_le32(cfg_data[84:])
        num_wds_entries = _create_le32(cfg_data[88:])
        dma_burst_size = _create_le32(cfg_data[92:])
        mac_aggr_delim = _create_le32(cfg_data[96:])
        rx_skip_defrag_timeout_dup_detection_check = _create_le32(cfg_data[100:])
        vow_config = _create_le32(cfg_data[104:])
        gtk_offload_max_vdev = _create_le32(cfg_data[108:])
        num_msdu_desc = _create_le32(cfg_data[112:])
        max_frag_entries = _create_le32(cfg_data[116:])
        num_tdls_vdevs = _create_le32(cfg_data[120:])
        num_tdls_conn_table_entries = _create_le32(cfg_data[124:])
        beacon_tx_offload_max_vdev = _create_le32(cfg_data[128:])
        num_multicast_filter_entries = _create_le32(cfg_data[132:])
        num_wow_filters = _create_le32(cfg_data[136:])
        num_keep_alive_pattern = _create_le32(cfg_data[140:])
        keep_alive_pattern_size = _create_le32(cfg_data[144:])
        max_tdls_concurrent_sleep_sta = _create_le32(cfg_data[148:])
        max_tdls_concurrent_buffer_sta = _create_le32(cfg_data[152:])
